...
使用 MySQL 8.0（生产）或 H2（开发），DDL 兼容两者。共 18 张表：9 张业务表 + 4 张知识库管理表 + 1 张检索日志表 + 1 张 Token 黑名单表 + 1 张定时任务表 + 2 张认证扩展表（字段）。

```sql
-- ============================================================
-- OfferPilot 数据库 Schema（v5.0 — 含认证 + 多租户知识库 + 薪资 + 安全）
-- ============================================================
...
-- 8. 用户长期记忆表（替代 MEMORY.md，按 userId 隔离）
...
前端用 Vue 3 + Element Plus，共 9 个核心页面（1 个登录页 + 4 个用户页面 + 4 个知识库管理页面）。
