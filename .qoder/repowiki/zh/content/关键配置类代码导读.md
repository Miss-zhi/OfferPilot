...
## 更新摘要
**变更内容**   
- **增强 AgentFactory 多提供商模型解析机制** - 新增 OpenAI 兼容 Provider 映射逻辑，支持 deepseek、siliconflow、volcengine 等第三方服务
- **完善 TranscriptionConfig 配置类** - 提供独立的录音转写服务配置，支持智能 API Key 回退机制
- **补齐 AgentScope 框架层 Provider 依赖** - 添加 dashscope、anthropic、gemini、ollama 等 Model Extension 依赖
- **优化 application.yml 兜底配置** - 使用正确的 dashscope 配置作为默认值
- **实现智能的 Provider 映射系统** - 将非原生支持的 Provider 自动映射到 openai 前缀
...
**Section sources**
- [AgentScopeProperties.java:53-82](file://src/main/java/com/tutorial/offerpilot/config/AgentScopeProperties.java#L53-82)
...
- [application.yml:36-41](file://src/main/resources/application.yml#L36-41)