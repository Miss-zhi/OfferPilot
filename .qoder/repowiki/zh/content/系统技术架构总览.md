...
- **已移除WebSearchFallbackService服务层兜底机制，网络搜索改为Agent层直接通过MCP协议调用**
...
WEBSEARCH["OpenWebSearch MCP Server<br/>端口: 3000 (SSE)"]
...
- [workspace/tools.json:1-11](file://workspace/tools.json#L1-L11)
...
### 兜底搜索机制重构
**已更新** WebSearchFallbackService已被删除，网络搜索机制从服务层自动兜底改为Agent层直接通过MCP协议调用。
...
**架构变更说明：**
- **旧架构**：服务层WebSearchFallbackService在知识库无结果时自动触发HTTP兜底搜索
- **新架构**：Agent层通过MCP协议直接调用web_search工具，由子Agent根据业务逻辑决定何时使用
- **优势**：更灵活的搜索策略控制，减少不必要的网络请求，提升整体性能

**章节来源**
- [workspace/tools.json:1-11](file://workspace/tools.json#L1-L11)
- [src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java:497-520](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L497-L520)
...
**章节来源**
- [src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java:61-73](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L61-73)
