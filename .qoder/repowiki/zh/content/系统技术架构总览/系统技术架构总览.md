...
## 更新摘要
**变更内容**   
- **架构重大重构**：从多子Agent架构（1主+7子）完全重构为单Agent全能助手架构，删除所有子Agent声明和分发逻辑
- **System Prompt彻底重写**：从"调度中心"角色完全重写为"全能助手"角色，定义详细的执行流程和工具使用规范
- **RAG召回策略全面升级**：实现行业标准的两阶段多路并行召回+RRF融合+Rerank精排方案
- **对话历史持久化系统**：新增完整的会话管理和消息持久化功能，支持SSE流式对话的完整记录
- **工具层大幅精简**：移除8个专用工具类和相关权限配置，精简为9个核心工具，权限控制大幅简化
- **MCP协议升级**：从HTTP JSON-RPC迁移到Streamable HTTP协议，提供更高效的实时通信能力
...
MAIN["单Agent全能助手<br/>HarnessAgent直接调用工具<br/>无需子Agent分发"]
...
WEBSEARCH["OpenWebSearch MCP Server<br/>端口: 3002 (Streamable HTTP)"]
end
FE --> SEC
SEC --> CTRL
CTRL --> SVC
...
### 全能助手Agent设计
**已重构** 系统从复杂的多子Agent架构完全重构为单一全能助手Agent，通过HarnessAgent直接管理所有工具调用，消除了子Agent间的通信开销和复杂性。

```mermaid
graph TB
MAIN["单Agent全能助手<br/>HarnessAgent - offerpilot_user"]
...
MCP["联网搜索<br/>web_search (MCP Streamable HTTP)"]
MAIN --> RESUME
MAIN --> INTERVIEW
MAIN --> SEARCH
MAIN --> MCP
...
- [src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java:212-309](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L212-309)

### System Prompt彻底重写
**已重写** System Prompt从"调度中心"角色完全重写为"全能助手"角色，定义了详细的执行流程和工具使用规范：
...
### 两阶段多路并行召回架构
**全新实现** 实现了行业标准的两阶段召回+精排架构，显著提升检索精度和性能：
...
**图表来源** 
- [src/main/java/com/tutorial/offerpilot/service/KnowledgeBaseService.java:267-280](file://src/main/java/com/tutorial/offerpilot/service/KnowledgeBaseService.java#L267-280)
- [src/main/java/com/tutorial/offerpilot/service/VectorSearchService.java:134-142](file://src/main/java/com/tutorial/offerpilot/service/VectorSearchService.java#L134-142)

### RRF融合算法实现
**全新实现** Reciprocal Rank Fusion (RRF) 多路召回融合算法，使用排名位置而非原始分数进行融合：
...
### Rerank精排服务
**全新实现** DashScope Rerank API封装，对候选结果进行语义相关性重排序：
...
**章节来源**
- [src/main/java/com/tutorial/offerpilot/service/RerankerService.java:47-80](file://src/main/java/com/tutorial/offerpilot/service/RerankerService.java#L47-80)
- [src/main/java/com/tutorial/offerpilot/service/KnowledgeBaseService.java:552-570](file://src/main/java/com/tutorial/offerpilot/service/KnowledgeBaseService.java#L552-570)

## 对话历史持久化系统

### 数据库架构设计
**全新实现** 完整的对话历史持久化机制，包含会话管理和消息存储：
...
**图表来源** 
- [src/main/java/com/tutorial/offerpilot/entity/ChatSession.java:12-14](file://src/main/java/com/tutorial/offerpilot/entity/ChatSession.java#L12-14)
- [src/main/java/com/tutorial/offerpilot/entity/ChatMessage.java:12-14](file://src/main/java/com/tutorial/offerpilot/entity/ChatMessage.java#L12-14)

### 会话管理功能
**全新实现** ChatHistoryService提供完整的会话CRUD操作：
...
**章节来源**
- [src/main/java/com/tutorial/offerpilot/service/ChatHistoryService.java:35-63](file://src/main/java/com/tutorial/offerpilot/service/ChatHistoryService.java#L35-63)
- [src/main/java/com/tutorial/offerpilot/service/ChatHistoryService.java:112-161](file://src/main/java/com/tutorial/offerpilot/service/ChatHistoryService.java#L112-161)

## 工具层精简

### 工具总数与分类
**已精简** 系统精简为9个@Tool注解工具，按功能分为三大类：
...
**章节来源**
- [src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java:315-343](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L315-343)

## MCP协议升级

### Streamable HTTP传输协议
**已升级** MCP协议从HTTP JSON-RPC迁移到Streamable HTTP协议，提供更高效的实时通信能力：
...
WEB_SEARCH_MCP["OpenWebSearch MCP Server<br/>端口: 3002"]
...
**图表来源** 
- [src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java:351-378](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L351-378)
...
WEBSEARCH["OpenWebSearch MCP<br/>3002 HTTP (Streamable)"]
...
**章节来源**
- [docker-compose.yml:15-117](file://docker-compose.yml#L15-117)
...
**章节来源**
- [src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java:61-72](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L61-72)

## 配置体系升级

### Rerank独立配置
**全新配置** Rerank配置与LLM模型配置完全解耦：
...
### MCP协议配置升级
**已升级** MCP协议配置从HTTP JSON-RPC迁移到Streamable HTTP协议：
...
- [src/main/java/com/tutorial/offerpilot/config/AgentScopeProperties.java:113-132](file://src/main/java/com/tutorial/offerpilot/config/AgentScopeProperties.java#L113-132)