...
- [ListKnowledgeBasesTool.java](file://src/main/java/com/tutorial/offerpilot/agent/tool/ListKnowledgeBasesTool.java)
- [InterviewModeService.java](file://src/main/java/com/tutorial/offerpilot/service/InterviewModeService.java)
</cite>

## 更新摘要
**变更内容**
- **架构重大重构**：从多子Agent架构完全简化为单Agent直接调用工具模式
- **移除已废弃功能**：删除薪资谈判、学习计划等依赖已删除服务的环节描述
- **重写所有Mermaid时序图**：全面反映新的单Agent架构和工具直调模式
- **精简工具集**：从8个子Agent + 13个工具简化为单Agent + 10个核心工具
- **统一权限控制**：所有工具均为ALLOW权限，无需细粒度权限管理
...
- **关键说明**
  ...
  - 工具返回"结构化数据 + 指导文本"，自然语言评分与建议由 LLM 在对话中动态生成
...
- [ResumeParseTool.java:21-26](file://src/main/java/com/tutorial/offerpilot/agent/tool/ResumeParseTool.java#L21-L26)
...
- **关键说明**
  ...
  - 评估流程：先调用 transcribe_audio 获取文字记录，再调用 analyze_answer 获取评估指导，最终由 LLM 生成具体分数与评语
  - 工具权限：transcribe_audio、analyze_answer 均配置为 ALLOW 权限
...
5. **精简工具集**：从13个工具简化为10个核心工具 + MCP联网搜索

### 已移除的子Agent
- ❌ resume_coach（简历教练）
- ❌ tech_evaluator（技术评估器）
- ❌ expression_evaluator（表达评估器）
- ❌ mock_interviewer（模拟面试官）
- ❌ company_researcher（公司研究员）
- ❌ study_planner（学习计划师）
- ❌ salary_advisor（薪资顾问）
...
### 保留的核心工具
- ✅ parse_resume（简历解析）
- ✅ evaluate_resume（简历评估）
- ✅ generate_next_question（出题指导）
- ✅ analyze_answer（回答分析）
- ✅ transcribe_audio（音频转写）
- ✅ search_questions（题目搜索）
- ✅ search_answers（答案搜索）
- ✅ smart_search（智能搜索）
- ✅ search_resources（资源搜索）
...
- [AgentFactory.java:212-309](file://src/main/java/com/tutorial/offerpilot/agent/AgentFactory.java#L212-309)
